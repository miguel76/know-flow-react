...
export function elementToFlow(flowFactory: FlowFactory, element: React.ReactElement):
...
export default function nodeToFlow(flowFactory: FlowFactory, node: React.ReactNode):
    ...
    if (node instanceof Types.Flow) {
        ...
        return flowFactory.createParallel(node.map(n => nodeToFlow(flowFactory, n)));
    } else {
        return flowFactory.createConstant(node);
    }
}

export function componentAttributesToFlow(flowFactory: FlowFactory, props: any):
